...
public class AttemptDaoO implements AutoCloseable{
	public Connection con = null;
	public AttemptDaoO() throws Exception{
	...
	public List<Attempts> displayScore() throws SQLException {
		...
		String sql = "SELECT attempt_id,quiz_id,quiz_attempts.user_id,name,final_score,total_questions FROM quiz_attempts INNER JOIN users ON quiz_attempts.user_id = users.user_id";
		try(PreparedStatement dt = con.prepareStatement(sql)) {
			...
			while(rs.next()) {
				...
				at.setStudent_name(rs.getString(4));
				at.setScore(rs.getInt(5));
				at.setTotal(rs.getInt(6));
				atmList.add(at);
			}
	...
	public boolean verifyAttempt(int qid) throws SQLException {
		String sql = "SELECT attempt_id FROM quiz_attempts WHERE quiz_id = ?";
		try(PreparedStatement dt = con.prepareStatement(sql)) {
			dt.setInt(1, qid);
			ResultSet rs = dt.executeQuery();
			if(rs.next()) {
